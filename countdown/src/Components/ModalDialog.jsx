...
const ModalDialog = forwardRef(function ModalDialog({timeRemaining, onResetTimeRemaining, timeTarget}, ref){
  const dialog= useRef();  
  ...
  const titleModal = userHasLost ? <>'¡Has perdido!'</> : <>Puntuacion total: <strong>{userScored}/100</strong></>;
  const timerIsStoppedText=userHasLost ? 
    <>No has conseguido parar el tiempo antes de la cuenta atras. ¡Vuelve a probar suerte!</> : 
    <>Has parado el tiempo a falta de <strong>{timeRemainingFormatted}</strong> segundos</>;
